package com.monta.library.ocpp.v16.core

import com.monta.library.ocpp.TestUtils
import com.monta.library.ocpp.common.serialization.Message
import com.monta.library.ocpp.common.serialization.MessageSerializer
import com.monta.library.ocpp.common.serialization.ParsingResult
import com.monta.library.ocpp.common.serialization.SerializationMode
import com.monta.library.ocpp.v16.error.OcppErrorResponderV16
import io.kotest.core.spec.style.StringSpec
import io.kotest.matchers.shouldBe
import io.kotest.matchers.types.shouldBeInstanceOf

class ChangeAvailabilitySerializationTest : StringSpec({
    val messageSerializer = MessageSerializer(SerializationMode.OCPP_1_6, OcppErrorResponderV16)

    "parse ChangeAvailability request" {
        val jsonString = TestUtils.getFileAsString("changeavailability/req.json")
        val parsingResult = messageSerializer.parse(jsonString)
        parsingResult.shouldBeInstanceOf<ParsingResult.Success<Message.Request>>()
        val ocppMessage = parsingResult.value
        ocppMessage.shouldBeInstanceOf<Message.Request>()
        ocppMessage.uniqueId shouldBe "req-1"
        ocppMessage.action shouldBe "ChangeAvailability"
        ocppMessage.payload shouldBe TestUtils.toJsonNode(
            "{" +
                "\"connectorId\":1," +
                "\"type\":\"Operative\"}"
        )
        val payloadResult = messageSerializer.deserializePayload(ocppMessage, ChangeAvailabilityRequest::class.java)
        payloadResult.shouldBeInstanceOf<ParsingResult.Success<ChangeAvailabilityRequest>>()
        payloadResult.value.connectorId shouldBe 1
        payloadResult.value.type shouldBe AvailabilityType.Operative
    }

    "serialize ChangeAvailability request" {
        val request = ChangeAvailabilityRequest(
            connectorId = 1,
            type = AvailabilityType.Operative
        )
        val message = Message.Request("req-1", "ChangeAvailability", messageSerializer.toPayload(request))
        message.toJsonString(messageSerializer) shouldBe "[2,\"req-1\",\"ChangeAvailability\",{\"connectorId\":1,\"type\":\"Operative\"}]"
    }

    "parse ChangeAvailability response" {
        val jsonString = TestUtils.getFileAsString("changeavailability/res.json")
        val parsingResult = messageSerializer.parse(jsonString)
        parsingResult.shouldBeInstanceOf<ParsingResult.Success<Message.Response>>()
        val ocppMessage = parsingResult.value
        ocppMessage.shouldBeInstanceOf<Message.Response>()
        ocppMessage.uniqueId shouldBe "res-1"
        ocppMessage.payload shouldBe TestUtils.toJsonNode("{\"status\":\"Accepted\"}")
        val payloadResult = messageSerializer.deserializePayload(ocppMessage, ChangeAvailabilityConfirmation::class.java)
        payloadResult.shouldBeInstanceOf<ParsingResult.Success<ChangeAvailabilityConfirmation>>()
        payloadResult.value.status shouldBe AvailabilityStatus.Accepted
    }

    "serialize ChangeAvailability response" {
        val confirmation = ChangeAvailabilityConfirmation(
            status = AvailabilityStatus.Accepted
        )
        val message = Message.Response("res-1", messageSerializer.toPayload(confirmation))
        message.toJsonString(messageSerializer) shouldBe "[3,\"res-1\",{\"status\":\"Accepted\"}]"
    }
})
